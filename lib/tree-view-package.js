...
class TreeViewPackage {
  ...
  consumeOpenExternal (service) {
    this.getTreeViewInstance().openExternalService = service
    return new Disposable(() => {
  ...
  consumeProjectList (projectList) {
    ...
    treeView.projectList = projectList
    if (treeView.addProjectsView) treeView.addProjectsView.setProjectList(projectList)
    return new Disposable(() => {
      if (this.treeView) {
