...
class Directory {
  constructor({name, fullPath, symlink, expansionState, isRoot, ignoredNames, useSyncFS, stats}) {
    ...
    if (atom.config.get('tree-panel.squashDirectoryNames') && !this.isRoot) {
  ...
  isPathIgnored(filePath) {
    if (atom.config.get('tree-panel.hideVcsIgnoredFiles')) {
      const repo = repoForPath(this.path)
  ...
  compareEntries(firstName, secondName) {
    return getCompareFn()(firstName, secondName)
  }
  ...
  sortEntries(combinedEntries) {
    if (atom.config.get('tree-panel.sortFoldersBeforeFiles')) {
      return combinedEntries
    } else {
      return combinedEntries.sort((first, second) => {
        return sortByBaseName(first, second)
      })
    }
  }
...
function getCompareFn () {
  return atom.config.get('tree-panel.sortMethod') === 'natural' ? naturalCompare : collatorCompare
}
...
function sortByBaseName (first, second) {
  const cmp = getCompareFn()
  if (atom.config.get('tree-panel.sortByBase')) {
