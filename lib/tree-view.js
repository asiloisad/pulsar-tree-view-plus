...
const { shell } = require(path.join(atom.getLoadSettings().resourcePath, 'node_modules', '@electron/remote'));
...
class TreeView {
  ...
  getPreferredLocation() {
    return atom.config.get("tree-view-plus.showOnRightSide") ? "right" : "left";
  }
  ...
  handleEvents() {
    ...
    atom.commands.add(this.element, {
      ...
      "tree-view:toggle-vcs-ignored-files": () =>
        toggleConfig(`tree-view-plus.hideVcsIgnoredFiles`),
      "tree-view:toggle-ignored-names": () => toggleConfig(`tree-view-plus.hideIgnoredNames`),
    ...
    this.disposables.add(
      ...
      atom.workspace.getCenter().onDidChangeActivePaneItem(() => {
        ...
        if (atom.config.get("tree-view-plus.autoReveal")) {
      ...
      atom.project.onDidAddRepository(() => this.updateRoots()),
      // …when any repo is destroyed (e.g. .git deleted) — rebuild so items lose their git colors.
      atom.project.observeRepositories((repo) => {
        this.disposables.add(repo.onDidDestroy(() => this.updateRoots()));
      }),
      // …when the user changes any of the settings that affect what gets shown
      // (and in what order).
      atom.config.onDidChange("tree-view-plus.hideVcsIgnoredFiles", () => this.updateRoots()),
      atom.config.onDidChange("tree-view-plus.hideIgnoredNames", () => this.updateRoots()),
      atom.config.onDidChange("core.ignoredNames", () => {
        if (atom.config.get("tree-view-plus.hideIgnoredNames")) {
          this.updateRoots();
        }
      }),
      atom.config.onDidChange("tree-view-plus.sortFoldersBeforeFiles", () => this.updateRoots()),
      atom.config.onDidChange("tree-view-plus.squashDirectoryNames", () => this.updateRoots()),
    );
  }
  ...
  entryClicked(event) {
    ...
    if (entry.classList.contains("directory")) {
      ...
      let doubleClick = atom.config.get("tree-view-plus.dirDoubleClick");
      if (doubleClick) {
        // Double-click mode
        if (detail === 1) {
          this.selectEntry(entry);
      ...
      let doubleClick = atom.config.get("tree-view-plus.fileDoubleClick");
      if (doubleClick) {
        // Double-click mode
        if (detail === 1) {
          this.selectEntry(entry);
          ...
          return this.openAfterPromise(entry.getPath(), {
            searchAllPanes: atom.config.get("tree-view-plus.alwaysOpenExisting"),
          });
        }
      } else {
  ...
  fileViewEntryClicked(event) {
    ...
    let { detail = 1 } = event;
    let alwaysOpenExisting = atom.config.get("tree-view-plus.alwaysOpenExisting");
  ...
  revealPath(filePath, options = {}) {
    ...
    focus ??= atom.config.get("tree-view-plus.focusOnReveal");
  ...
  openSelectedEntry(options = {}, expandDirectory = false) {
    ...
    if (selectedEntry.classList.contains("directory")) {
      ...
      if (atom.config.get("tree-view-plus.alwaysOpenExisting")) {
    ...
    atom.confirm(
      ...
      async (response) => {
        if (response === 0) {
          ...
          if (atom.config.get("tree-view-plus.squashDirectoryNames")) {
  ...
  add(isCreatingFile) {
    ...
    dialog.onDidCreateDirectory((createdPath) => {
      ...
      if (atom.config.get("tree-view-plus.squashDirectoryNames")) this.updateRoots();
