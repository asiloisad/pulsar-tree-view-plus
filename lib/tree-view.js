...
class TreeView {
  ...
  getPreferredLocation() {
    return atom.config.get('tree-panel.showOnRightSide') ? 'right' : 'left';
  }
  ...
  isPermanentDockItem() {
    return atom.config.get('tree-panel.isPermanent');
  }
  ...
  handleEvents() {
    ...
    atom.commands.add(this.element, {
      ...
      'tree-view:toggle-vcs-ignored-files': () => toggleConfig(`tree-panel.hideVcsIgnoredFiles`),
      'tree-view:toggle-ignored-names':     () => toggleConfig(`tree-panel.hideIgnoredNames`),
    ...
    this.disposables.add(
      ...
      atom.workspace.getCenter().onDidChangeActivePaneItem(() => {
        ...
        if (atom.config.get('tree-panel.autoReveal')) {
      ...
      atom.config.onDidChange(
        'tree-panel.hideVcsIgnoredFiles',
        () => this.updateRoots()
      ),
      atom.config.onDidChange(
        'tree-panel.hideIgnoredNames',
        () => this.updateRoots()
      ),
      atom.config.onDidChange(
        'core.ignoredNames',
        () => {
          if (atom.config.get('tree-panel.hideIgnoredNames')) {
            this.updateRoots();
          }
        }
      ),
      atom.config.onDidChange(
        'tree-panel.sortFoldersBeforeFiles',
        () => this.updateRoots()
      ),
      atom.config.onDidChange(
        'tree-panel.squashDirectoryNames',
        () => this.updateRoots()
      )
    );
  }
  ...
  entryClicked(event) {
    ...
    if (entry.classList.contains('directory')) {
      let doubleClick = atom.config.get('tree-panel.dirDoubleClick');
      if (doubleClick) {
        // Double-click mode
        if (detail === 1) {
      ...
      let doubleClick = atom.config.get('tree-panel.fileDoubleClick');
      if (doubleClick) {
        // Double-click mode
        if (detail === 1) {
          this.selectEntry(entry);
          ...
          return this.openAfterPromise(entry.getPath(), {
            searchAllPanes: atom.config.get('tree-panel.alwaysOpenExisting')
          });
        }
      } else {
  ...
  fileViewEntryClicked(event) {
    ...
    let { detail = 1 } = event;
    let alwaysOpenExisting = atom.config.get('tree-panel.alwaysOpenExisting');
    ...
    if (!atom.config.get('tree-panel.fileDoubleClick')) detail = 1;
    if (detail === 1) {
  ...
  revealActiveFile(options = {}) {
    ...
    focus ??= atom.config.get('tree-panel.focusOnReveal');
  ...
  openSelectedEntry(options = {}, expandDirectory = false) {
    ...
    if (selectedEntry.classList.contains('directory')) {
      ...
      if (atom.config.get('tree-panel.alwaysOpenExisting')) {
  ...
  async removeSelectedEntries() {
    ...
    atom.confirm({
      ...
      if (response === 0) { // Move to Trash
        ...
        if (atom.config.get('tree-panel.squashDirectoryNames')) {
