...
class TreeView {
  constructor(state) {
    ...
    this.disposables.add(
      ...
      this.onWillDeleteEntry(({ pathToDelete }) => {
        ...
        for (let editor of atom.workspace.getTextEditors()) {
          let filePath = editor.getPath();
          if ((isDir ? filePath?.startsWith(pathToDelete) : filePath === pathToDelete) && !editor.isModified()) {
  ...
  async removeSelectedEntries() {
    ...
    selectedEntries = Array.from(selectedEntries).filter(e => !e.closest('.special-root-section'));
