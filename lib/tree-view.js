...
class TreeView {
  ...
  isPermanentDockItem() {
    return atom.config.get('tree-view-plus.isPermanent');
  }
  ...
  handleEvents() {
    ...
    atom.commands.add(this.element, {
      ...
      'tree-view:toggle-vcs-ignored-files': () => toggleConfig(`tree-view-plus.hideVcsIgnoredFiles`),
    ...
    this.disposables.add(
      ...
      atom.config.onDidChange(
        'tree-view-plus.hideVcsIgnoredFiles',
        () => this.updateRoots()
      ),
      atom.config.onDidChange(
        'tree-view-plus.hideIgnoredNames',
        () => this.updateRoots()
      ),
      atom.config.onDidChange(
        'core.ignoredNames',
      ...
      atom.config.onDidChange(
        'tree-view-plus.sortFoldersBeforeFiles',
        () => this.updateRoots()
      ),
      atom.config.onDidChange(
        'tree-view-plus.squashDirectoryNames',
        () => this.updateRoots()
      )
    );
  }
  ...
  fileViewEntryClicked(event) {
    ...
    // In single-click mode, treat every click as a single click
    if (!atom.config.get('tree-view-plus.fileDoubleClick')) detail = 1;
    if (detail === 1) {
  ...
  onMouseDown(event) {
    ...
    // Middle-click: select only, prevent auto-scroll
    if (event.button === 1) {
